...
# 主键约定(2.0)：目录表用 数字id(跨表引用) + code(英文代码,程序用)；icon 只填基名。
COMMENTS = {
    "buildings.csv": [
        "数字ID(跨表引用用)", "代码标识(英文,勿改)", "显示名", "图标基名(渲染时拼 /art/+基名+.png)",
        "木材基价", "泥土基价", "铁矿基价", "粮食基价", "造价增长率/级", "建造基础秒数",
        "时间增长率/级", "最高等级", "前置(用建筑数字ID:等级,如 4:3,多个用|)",
    ],
    "fields.csv": [
        "数字ID(跨表引用用)", "代码标识(英文,勿改)", "显示名", "图标基名(渲染时拼 /art/+基名+.png)",
        "产出资源", "每级产量基数", "产量增长率/级", "木材基价", "泥土基价", "铁矿基价",
        "粮食基价", "造价增长率/级", "升级基础秒数", "时间增长率/级", "最高等级",
    ],
    "units.csv": [
        "数字ID(跨表引用用)", "代码标识(英文,勿改)", "所属部族(语义串)", "显示名",
        "图标基名(渲染时拼 /art/+基名+.png)", "兵种类别", "攻击力", "对步防御", "对骑防御",
        "移动速度", "负重(掠夺量)", "每小时耗粮", "木材造价", "泥土造价", "铁矿造价",
        "粮食造价", "训练秒数/个", "所需建筑(建筑数字ID)",
    ],
    "resources.csv": [
        "资源标识(语义串,代码与存档用,勿改)", "显示名", "图标基名(渲染时拼 /art/+基名+.png)", "用途备注",
    ],
    "pve_targets.csv": [
        "数字ID(跨表引用用)", "代码标识(英文,勿改)", "显示名", "图标基名(渲染时拼 /art/+基名+.png)",
        "重生秒数", "掠夺木材", "掠夺泥土", "掠夺铁矿", "掠夺粮食",
    ],
    "pve_defenders.csv": [
        "所属野怪类型(野怪数字ID)", "守军兵种代码(此目标内部标签,不跨表)", "显示名", "数量",
        "攻击力", "对步防御", "对骑防御", "负重",
    ],
    "pve_spawns.csv": [
        "刷新点标识(实例ID,勿改)", "野怪类型(野怪数字ID)", "地图X坐标", "地图Y坐标",
    ],
}
