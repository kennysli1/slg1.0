...
测试覆盖：15 个服务端测试文件，76 项用例，覆盖单人全循环、多人 PvP、人口系统、持久化、配置校验、manifest 与架构守卫。
...
| 加新系统（工会/邮件等） | 照 `packages/server/src/modules/` 模板加模块，挂到 `app.ts`，写 `static MANIFEST` 并在 `gateway.ts` 的 `MODULE_MANIFESTS` 登记（不必手改路由表） |
