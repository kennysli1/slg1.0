...
5. 配置中心会读取 PR 的 `mergeable`、检查和冲突文件状态；`/config` 页面会把“已上传”“检查中”“存在冲突”“可以合并”“已合并”分开显示。冲突文件可在页面中查看对应权威源、`main` 和 PR 当前版本，人工编辑最终 CSV 后通过 `POST /config/sync/resolve` 生成包含最新 `main` 的双父提交。`dialogues.csv` 的权威源是配置中心；`units.csv` 的权威源是服务器当前文件；其余表默认由配置中心维护。提交前会做整图配置校验。

发布时由各表声明的权威源负责现有生产值，Git 只负责结构和从未出现过的默认值。`tools/remote-release.sh` 调用 `scripts/merge-persisted-config.mjs`，以各表稳定主键合并：对话使用配置中心共享 CSV（包括明确空值），兵种在创建新 release 前先从服务器当前 release 镜像到共享 CSV，再参与合并；同步 PR 尚未合并的新增行也继续保留。`units.csv` 不允许从配置中心编辑，避免旧配置反向覆盖服务器兵种。编辑器删除行时会在 `shared/config/config_row_tombstones.json` 留下稳定主键，后续部署不得从 Git 复活该行；重新添加同一主键会清除删除记录。真正的结构删除或改名必须走显式迁移，不能借代码部署静默删除生产配置。
...
- 生产与 GitHub 哈希不一致：先按文件权威源判断——对话以配置中心共享 CSV 为准，兵种以服务器当前 release 为准；在 `/config` 刷新/同步后再记录 release SHA 和 revision，禁止用旧 Git CSV 反向覆盖权威值。
