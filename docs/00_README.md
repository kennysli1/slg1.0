# Travian 2.0 项目

> 📌 **理解整个项目请先看仓库根的 `PROJECT.md`**（项目入口：是什么/目录/架构/模块/怎么配置扩展修改运行）。本文是 `docs/` 文档区的索引，侧重设计过程与文档导航。

> 网页游戏，以 Travian 为核心做扩展修改。核心诉求：**先把系统接口/架构跟人商量定死，再开发**，避免 AI 一把梭写死、系统间耦合不清、后续难改。

## 三阶段流程（按此推进）

```
阶段一 · GDD       不重新发明。以 Travian 现成系统清单为基线，逐系统标【保留/修改/新增】
   │               （Travian 是验证过能跑的完整设计，是我们的 GDD 底稿）
   ▼
阶段二 · 架构      ★重点★ 由 AI 据 GDD 产出：模块职责 + 模块间接口契约（传什么数据/事件）
   │               + 数据归属 + 扩展点。这层决定"以后好不好改"，慢工细做。
   ▼
阶段三 · 代码      架构签字后才动手。
```

**关键原则**：自顶向下锁接口，不自底向上抠玩法细节。玩法细节（如资源几种）等架构成型后再填。

## 目录

```
docs/
├── 1_原版拆解/          参照系：Travian 反向拆解（可信，作 GDD 基线）
│   ├── 设计/            01核心框架 02资源建筑 03军事战斗 04社交终局 05数值 08系统逻辑详解
│   └── 开发计划/        07 原版技术架构反推
│
└── 2_2.0设计/           我们的 2.0
    ├── 01_定位与改动方针.md     已敲定的总方针（见下）
    └── 改进方向备选池.md        待选的扩展点，不急着定

服务器/                   数据存储结构 + 数据库操作手册（备份/刷档/删档/换DB）
```

## 已敲定的方针（阶段一基调）

1. 先把**基础循环**做出来、做有趣；玩家定位/优化/社交/赛季后置。
2. **PvE 与 PvP 共用同一套养成 + 战斗数值**：一支军队一套数值；PvE=稳定发育，PvP=掠夺加速，战利品回流同一经济。
3. 先**单人**，不做社交/联盟。
4. 先不承诺**赛季制**，先跑通一个局内玩法。

## 下一步

阶段一、阶段二（架构+通信格式）已定稿。下一步可进入：
- **阶段二补完**（可选）：逐模块状态归属细化（按需，不必逐命令抠 payload）。
- **阶段三 代码**：搭框架骨架——基础设施层(Scheduler/EventBus/Persistence) + Gateway + 一个最小领域模块走通一条端到端链路。

## 进度

- [x] 阶段一 GDD：系统清单定稿（`2_2.0设计/02_系统清单.md`）
- [x] 阶段二 架构总览定稿（`03_架构总览.md`）
- [x] 阶段二 通信格式规范定稿（`04_通信格式规范.md`）
- [x] 阶段三 技术栈定稿（`05_技术栈与工程结构.md`）
- [x] **阶段三 骨架链路跑通**：基础设施(EventBus/Scheduler/Store/CommandBus) + Gateway/WS + Economy/Building + 前端演示页。升级建筑全链路经单元测试(4/4)、WebSocket、浏览器三重实测通过。
- [x] 代码导读 + 扩展与代码规范定稿（`06`、`07`）
- [x] **文字版 Travian 完整可玩**：6 大模块(Economy/Building/Military/World/Movement/Combat/PvE) + 完整网页前端。核心循环跑通：4资源经济→训练罗马全兵种→出征打PvE→掠夺→返程入库。全循环测试 4/4 通过。
- [ ] 美术资源生成替换（清单见 `美术资源清单.md`）
- [ ] 部署到腾讯云（见 `部署手册_腾讯云轻量服务器.md`）

## 怎么玩（本地）

```bash
npm install                      # 首次
npm run build:shared             # 改过 shared 后必跑
npm run dev:server               # 终端A：后端 :8080
npm run dev -w @slg/client       # 终端B：前端，打开提示的 http://localhost:5173
```
四个标签页：🏠村庄(升级18资源田+10建筑) / ⚔️军队(训练罗马10兵种) / 🗺️地图(出征掠夺6个PvE目标) / 📜报告(战报)。

## 配置表速查（改数值在哪改）

所有游戏数值在仓库根 `config/` 的 CSV 里，Excel 可改，改完重启后端生效，**不动代码**。每张表配什么：

| # | 文件 | 配什么 |
|---|------|--------|
| 表1 | `config/resources.csv` | **资源种类**（木/泥/铁/粮） |
| 表2 | `config/fields.csv` | **资源田**（伐木场/采泥场/铁矿/农田 的产量·成本·时间） |
| 表3 | `config/buildings.csv` | **中心建筑**（10个建筑的成本·时间·科技树前置） |
| 表4 | `config/units.csv` | **兵种**（罗马10兵种的攻防·速度·载货·耗粮·造价） |
| 表5 | `config/pve_targets.csv` | **野怪/PvE目标**（战利品·重生时间） |
| 表6 | `config/pve_defenders.csv` | **野怪守军**（每个目标里有哪些怪·几只·多强） |
| 表7 | `config/pve_spawns.csv` | **野怪在地图的位置**（坐标分布） |

> 每张表**每一列**的详细含义、以及常见改法举例，见 `config/README.md`。
