...
building.Demolish → trade/treasure/mercenary/research 清理 + 重分布
research.TechCompleted → economy.SetRateModifier + military.SetTechCombatMult + population.SetTechGrowthMult
building.Built/Upgraded → research.onAcademyChanged（重算学院参数 + 重调度 RP tick）
```
...
| 科研 | `test/research.test.ts`（8 例：RP生产/科技树/依赖/GM覆盖层） |
