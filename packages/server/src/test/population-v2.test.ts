 ...
 *  C1  三部族 createVillage 满员：currentPop/hardCap ≥ 0.99，softLimit===availableLabor
 *  C2  新村劳动占比=1.0（满员）→ 繁荣度加成满值 → prosperityMult=1.0，五轴统一
 *  C3  全部兵种 cropPerHourEach>0（士兵以 upkeep 计入口粮，训练严格增加粮食压力）
 *  C4  增长朝 availableLabor 线性收敛：growthPerHour = popGrowthPerHour×mainLevel，夹在缺口内
...
for (const tribe of ['romans', 'gauls', 'teutons'] as const) {
  test(`v3 C1 [${tribe}] createVillage 满员：currentPop/hardCap ≥ 0.99 且 softLimit===availableLabor`, async () => {
    const app = freshApp();
    ...
    const ratio = snap.currentPop / snap.hardCap;
    assert.ok(ratio >= 0.99 && ratio <= 1.0001, `[${tribe}] currentPop/hardCap=${ratio} 应≥0.99`);
    assert.equal(snap.softLimit, snap.availableLabor, 'v3 兼容别名 softLimit 应等于 availableLabor');
    assert.ok(Math.abs(snap.prosperityMult - 1.0) < 1e-6, `[${tribe}] 满员 prosperityMult 应=1.0，实际 ${snap.prosperityMult}`);
...
test('v3 C2：新村劳动占比=1.0（满员）→ 繁荣度加成满值，五轴统一=1.0', async () => {
  const app = freshApp();
  const vid = await reg(app, 'c2', 'romans');
  const snap = (await send(app, 'population.GetSnapshot', { villageId: vid })).payload as any;
  assert.ok(Math.abs(snap.laborRatio - 1.0) < 1e-6, `新村 laborRatio 应=1.0，实际 ${snap.laborRatio}`);
  assert.ok(Math.abs(snap.prosperityBonus - 1.0) < 1e-6, '繁荣度加成应满值 1.0');
  assert.ok(Math.abs(snap.prosperityMult - 1.0) < 1e-6, 'prosperityMult 应=1.0');
  for (const axis of ['production', 'build', 'train', 'research', 'smithy'] as const) {
    ...
    assert.ok(Math.abs(snap.laborMults[axis] - 1.0) < 1e-6, `${axis} 倍率应=1.0，实际 ${snap.laborMults[axis]}`);
  }
});
...
test('v3 C4：增长朝 availableLabor 线性收敛（popGrowthPerHour × mainLevel）', async () => {
  const app = freshApp();
  ...
  // 满员开局：缺口=0 → 增速=0
  assert.equal(snap0.growthPerHour, 0, '满员开局 growthPerHour 应为0');

  // 直接扣 30 劳动人口（模拟出征/训练消耗），制造增长缺口
  const cRes = await send(app, 'population.ConsumePop', { villageId: vid, unit: 'legionnaire', count: 30 });
  ...
  const expectedGrowth = Math.min(gap, app.config.constants.popGrowthPerHour * mainLevel);
  assert.ok(
    Math.abs(snap1.growthPerHour - expectedGrowth) < 1e-6,
    `growthPerHour 应为 min(gap, popGrowthPerHour*mainLevel)=${expectedGrowth}，实际 ${snap1.growthPerHour}`,
  );

  // 快进 1 小时：人口应增长（不超 growthPerHour）
  tick(3_600_000);
...
test('v3 C7：粮荒停止后发出 recovery 事件，inFamine 变 false', async () => {
  ...
  const modestUpkeep = initPop * c7c.popCropPerLabor * 2;
  await send(app, 'economy.SetBaseRate', { villageId: vid, resource: 'crop', ratePerHour: 0.001 });
