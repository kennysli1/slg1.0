...
test('科技树查询返回配置的 21 个科技 + 正确的 status', async () => {
  const app = freshApp();
  ...
  assert.equal(techs.length, 21, `应有21个科技，实际: ${techs.length}`);
  const t1 = techs.find((t: any) => t.code === 'military_drill');
  assert.ok(t1, 'military_drill 应存在');
...
test('StartResearch 无学院应被拒（academy_required）', async () => {
  ...
  const va = (regRes.payload as any).player.villageId;
  const r = await send(app, 'research.StartResearch', { villageId: va, techCode: 'military_drill' });
...
test('GM 覆盖层：research 表编辑 round-trip', async () => {
  ...
  assert.equal(r.ok, true);
  const t = (r.payload as any).techs.find((t: any) => t.code === 'military_drill');
  assert.equal(t.rpCost, 2, 'military_drill 默认 rpCost=2');
  assert.equal(t.durationSec, app.config.research.military_drill.durationSec, 'military_drill durationSec 应来自配置中心');
});
...
test('依赖链：有前置的科技在未满足时 locked', async () => {
  ...
  const va = (regRes.payload as any).player.villageId;
  const r = await send(app, 'research.GetTechTree', { villageId: va });
  const elite = (r.payload as any).techs.find((t: any) => t.code === 'advanced_arms');
  assert.ok(elite, 'advanced_arms 应存在');
...
test('科研配置校验 — 无环依赖', () => {
  ...
  assert.ok(r['military_drill'], 'military_drill 存在');
  assert.ok(r['advanced_arms'], 'advanced_arms 存在');
...
test('研究：学院拆除暂停后取消，仍按原总时长计算九折退款', async () => {
  ...
  const va = (regRes.payload as any).player.villageId;
  const tech = app.config.research.military_drill;
