...
- 恢复两条被部署 reset 丢失的科技（从 `_auth` 副本 git 历史重建）：**全民皆兵**（`universal_conscription`，完成时 `population.SetConscriptionMult` 提升动员上限比例）+ **露天仓库**（`open_warehouse`，`storage_overflow` 机制 + `economy.SetOverflowCap` 溢出系数 + 满仓时人口增长扣减；**无该科技时 `economy.Grant` 超额资源被丢弃，有科技可溢出至 `capacity×(1+overflowCap)`**，`productionPaused` 改为满仓即停产）。同时恢复：`build_speed` 科技效果注入（`building.SetBuildSpeedMult` 缓存 + 建造时间/估算生效）、`research` 重启后重新 apply 已完成科技效果、科技/宝物效果类型启动期白名单校验、`research.GetTechMult` 查询命令。同步更新 economy-overflow/found-village/transport-abandon/trade 测试以对齐新的超额语义。
