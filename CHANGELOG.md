...
- **[需刷档]** 未探索格不再允许野外驻扎：只能派军探索，按集结点等级限制可踏入的未探索深度（1级为1格），抵达或遇障均立即返程。军队在每个行军节点的视野会由服务器自动写入已探索历史。

- **[需刷档]** 新增野外驻扎行军：可向任意地图坐标派军驻扎，目标到达前被设施或军队占据时自动停在前一格；驻扎军可召回或继续驻扎、掠夺、攻城。集结点等级决定行军点上限，每支离城军持续占用一个行军点。

- **[需刷档]** 新增服务器权威的战争迷雾：城池与在途军队共同提供视野；地块区分未探索、已探索和当前可见，已探索仅保留最后一次观测快照。兵种增加可配置视野属性，军队按数量最多的兵种（并列取视野最大）决定视野。
