...
- PvP 建筑结算区分“破坏”和“拆除”：普通部队只破坏城外建筑等级并保留可修复状态，防守方可按当前等级到首次受损前等级的累计资源、累计时间三分之一一次性修复；受损建筑不能升级但仍可手动拆除。攻城武器先于普通部队结算拆除，降至 0 级时完全移除并产生建筑拆除战利品；攻城时城内建筑只接受攻城武器拆除。战报明确标记每条建筑的破坏/拆除及等级变化。
