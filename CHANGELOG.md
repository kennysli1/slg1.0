...
- 新增城内保险库建筑（最高 10 级）：`building_levels.csv` 为每级配置木材、泥土、铁、粮食和金币的增量保护量，GM 建筑卡片可编辑并通过持久化平衡覆盖保存，删档不会清除。攻城结算先按幸存攻城武器拆除建筑，再读取拆除后的保险库等级扣除保护额，最后装载仓库/粮仓战利品；保险库被拆后同一场攻城立即暴露更多资源。
