...
> **权威来源**：`dialogues.csv` 以配置中心为准；`units.csv` 以服务器当前 release 为准，配置中心只读。服务器直接更新兵种后，在配置中心点击“刷新配置同步状态”或“立即同步 / 重试”会镜像并排队同步；发布前也会自动捕获服务器当前兵种，避免旧 Git CSV 覆盖。
...
| 表4 | `units.csv` | **兵种**（罗马/高卢/条顿；`all` 为通用兵种） | **服务器权威**；配置中心只读，服务器直接更新后在 `/config` 刷新/同步；`simTraits` 为独立阶段化模拟器特性引用；冒险者为 `all` 通用侦察兵种 |
